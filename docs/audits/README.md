...
- [AUDIT_STRUCTURE_TESTS_MECANIQUE.md](phase4-tests-fond/AUDIT_STRUCTURE_TESTS_MECANIQUE.md) couvrait les tests existants au moment de sa rédaction ; le fichier `unauth.guard.spec.ts` (ajouté après, cf. [DETTE_ET_SUIVI.md](../../DETTE_ET_SUIVI.md) P10-01) n'y était pas inclus. **Réserve levée** : `unauth.guard.spec.ts` a depuis été vérifié contre la même grille (catégories A et D applicables — B, C, E hors périmètre pour ce fichier, comme pour tout fichier sans `HttpTestingController`/non-Cypress/non-JUnit) : 0 défaut trouvé.
- [AUDIT_TYPAGE_METHODES.md](ex1-conformite/AUDIT_TYPAGE_METHODES.md) affirme « 37 méthodes toutes typées explicitement », mais le tableau nommé de l'audit lui-même en énumère 38 (incohérence interne de rédaction, sans lien avec une évolution du code). **Réserve levée** : les 38 méthodes listées ont été revérifiées individuellement (pas par échantillon) contre le code actuel — 38/38 toujours typées explicitement, aucune régression.
